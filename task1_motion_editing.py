...
def part1_translation_and_rotation(viewer, setting_id):
    # 一些不同的设置
    bvh_list = [
        'motion_material/walk_forward.bvh', 'motion_material/run_forward.bvh',
    ...
    new_motion = original_motion.translation_and_rotation(
        frame, pos, facing_xz)

    translation, orientation = new_motion.batch_forward_kinematics()
    task = ShowBVHUpdate(viewer, new_motion.joint_name, translation,
                         orientation)
...
def part2_interpolate(viewer, v):
    ...
    run_forward = run_forward.translation_and_rotation(0, np.array([0, 0]),
                                                       np.array([0, 1]))

    # 计算插值系数
    v1 = (walk_forward.joint_position[-1, 0, 2] /
          walk_forward.motion_length) * 60
    v2 = (run_forward.joint_position[-1, 0, 2] /
          run_forward.motion_length) * 60
    blend_weight = (v - v1) / (v2 - v1)
    distance = (1 - blend_weight) * walk_forward.joint_position[
        -1, 0, 2] + blend_weight * run_forward.joint_position[-1, 0, 2]
...
def part4_concatenate(viewer, setting_id):
    if setting_id == 0:
        ...
        pos = motion.joint_position[-1, 0, [0, 2]]
        rot = motion.joint_rotation[-1, 0]
        facing_axis = R.from_quat(rot).apply(np.array([0, 0,
                                                       1])).flatten()[[0, 2]]
        new_motion = motion.translation_and_rotation(0, pos, facing_axis)
